...
领取风控会把当前请求计入时间窗口次数。默认时间窗口为 10 秒、最大操作数为 5；当 `recent_count >= 5` 时执行拦截，因此第 5 次请求即触发风控，而不是等到第 6 次。两个参数仍可通过 `RISK_CLAIM_WINDOW_SECONDS` 和 `RISK_MAX_CLAIMS_IN_WINDOW` 环境变量调整。
