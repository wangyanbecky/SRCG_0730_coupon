"""Operator blueprint - campaign management, risk review, notifications."""
from datetime import datetime
from flask import (Blueprint, render_template, request, redirect,
                   url_for, flash, jsonify)
from flask_login import login_required, current_user
from app import db
from app.models import Campaign, RiskLog, User, Notification, Coupon
...
def dashboard():
    """Operator main dashboard - list all campaigns."""
    campaigns = Campaign.query.order_by(Campaign.created_at.desc()).all()
    active_count = sum(1 for c in campaigns if c.status == 'active')
    draft_count = sum(1 for c in campaigns if c.status == 'draft')
...
def create_campaign():
    ...
    if request.method == 'POST':
        ...
        if errors:
            for e in errors:
                flash(e, 'error')
            return render_template('operator/campaign_form.html',
                                 campaign=None, is_edit=False)

        # If scheduled, use the scheduled_time as the campaign start_date
        # so users can see it automatically when the time arrives.
        # Always set status to 'active' — start_date/end_date already
        # control when the campaign is claimable.
        if is_scheduled and scheduled_time:
            start_date = scheduled_time

        campaign = Campaign(
            name=name, amount=amount, stock=stock, initial_stock=stock,
            start_date=start_date, end_date=end_date,
            per_user_limit=per_user_limit, description=description,
            status='active', is_scheduled=is_scheduled,
...
def edit_campaign(campaign_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            campaign.per_user_limit = int(request.form.get('per_user_limit', campaign.per_user_limit))
            # Coupon validity days
            cv_str = request.form.get('coupon_validity_days', '').strip()
            campaign.coupon_validity_days = int(cv_str) if cv_str else None
        except ValueError:
            flash('数值格式不正确。', 'error')
            return render_template('operator/campaign_form.html',
                                 campaign=campaign, is_edit=True)
...
def campaign_insights(campaign_id):
    """View campaign insights - claim statistics."""
    campaign = Campaign.query.get_or_404(campaign_id)
    coupons = campaign.coupons.order_by(Coupon.claimed_at.desc()).all()

    claimed = [c for c in coupons if c.status == 'claimed']
    verified = [c for c in coupons if c.status == 'verified']
    expired = [c for c in coupons if c.status == 'expired']

    # Calculate hourly claim distribution (for chart)
    from collections import Counter
    hour_dist = Counter()
    for c in coupons:
        if c.claimed_at:
            hour_dist[c.claimed_at.strftime('%m-%d %H:00')] += 1
...
def risk_logs():
    """View and manage risk/flag logs."""
...
def resolve_risk_log(log_id):
    """Resolve a risk flag (approve or reject)."""
...
def notifications():
    ...
    return render_template('operator/notifications.html',
                         users=users,
